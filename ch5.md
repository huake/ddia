...
本章将假设你的数据集非常小，每台机器都可以保存整个数据集的副本。在[第6章](ch6.md)中将放宽这个假设，讨论对单个机器来说太大的数据集的分割（分片,分片有可能跨机房，导致在mr在reduce的时候出现拉取数据失败的情况，特别是数据量大的时候。比如：存储节点跨机房，分到同一个region，在写这个afs集群的时候，出现写超时！）。在后面的章节中，我们将讨论复制数据系统中可能发生的各种故障，以及如何处理这些故障。
...
（凤巢采用相似的架构，凤巢会有异步从库，异步从库提供读访问权限，会有分库分表从操作）
